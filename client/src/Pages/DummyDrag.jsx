...
// Handle Drag End for both sections and files
const onDragEnd = (result) => {
    ...
    // Reorder sections
    if (type === 'section') {
      const updatedSections = Array.from(investorData[activeTab]);
    ...
    // Reorder files inside the accordion section
    if (type === 'file') {
      const updatedSections = Array.from(investorData[activeTab]);
      const sectionIndex = source.droppableId;

      const updatedFiles = Array.from(updatedSections[sectionIndex].items);
      const [movedFile] = updatedFiles.splice(source.index, 1);
      updatedFiles.splice(destination.index, 0, movedFile);

      updatedSections[sectionIndex] = { ...updatedSections[sectionIndex], items: updatedFiles };
...
return (
    <DragDropContext onDragEnd={onDragEnd}>
      ...
      <div>
        ...
        <Droppable droppableId={activeTab} type="section">
          {(provided, snapshot) => (
            <AccordionGroup
              sx={{
                minWidth: 400,
            ...
            >
              {investorData[activeTab] && investorData[activeTab].length > 0 ? (
                investorData[activeTab].map((key, id) => (
                  <Draggable key={id} draggableId={key.title} index={id}>
                    {(provided) => (
                      <Accordion
                        ref={provided.innerRef}
                      ...
                      >
                        ...
                        <AccordionDetails>
                          <Droppable droppableId={String(id)} type="file">
                            {(provided) => (
                              <div ref={provided.innerRef} {...provided.droppableProps}>
                                {key.items.map((element, itemId) => (
                                  <Draggable
                                    key={itemId}
                                    draggableId={element.heading}
                                    index={itemId}
                                  >
                                    {(provided) => (
                                      <AccordionDetails
                                        ...
                                        {...provided.dragHandleProps}
                                        onClick={() =>
                                          callMeToEditFile(element, key.title)
                                        }
